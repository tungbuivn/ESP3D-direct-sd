...
void handle_web_interface_root()
{
    ...
    if ((SPIFFS.exists(pathWithGz) || SPIFFS.exists(path)) && !web_interface->web_server.hasArg("forcefallback") && web_interface->web_server.arg("forcefallback") != "yes")
    {
        ...
        {
            file = SPIFFS.open(path, SPIFFS_FILE_READ);
        }
...
void handleSDFileList()
{
    ...
    if (web_interface->web_server.hasArg("action"))
    {
        ...
        if (web_interface->web_server.arg("action") == "deletedir" && web_interface->web_server.hasArg("filename"))
        {
            ...
            if (filename != "/")
            {
                bool delete_success = SD.rmdir(filename);
...
void SPIShareSdCardUpload()
{
    ...
    {
        // TODO: send command release sdcard to printer
        ...
        if ((web_interface->_upload_status != UPLOAD_STATUS_FAILED) || (upload.status == UPLOAD_FILE_START))
        {
            if (upload.status == UPLOAD_FILE_START)
            {
                ...
                else
                {
                    // if no set cancel flag
                    web_interface->_upload_status = UPLOAD_STATUS_FAILED;
                    ESPCOM::println(F("Can not begin sd file"), PRINTER_PIPE);
                    pushError(ESP_ERROR_FILE_CREATION, "Can not begin sd file");
                }
            }
    ...
    else
    {
        ...
        if ((web_interface->_upload_status != UPLOAD_STATUS_FAILED) || (upload.status == UPLOAD_FILE_START))
        {
            ...
            { // if com error no need to send more data to serial
                for (uint pos = 0; (pos < upload.currentSize) && (web_interface->_upload_status == UPLOAD_STATUS_ONGOING); pos++)
